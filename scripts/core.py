...
previsão e assinatura de entrega. Ele não sabe nada sobre negócios — quem
sabe é o *kit* de cada atividade (``kit_a01.py``, ``kit_a02.py``, ...).

    ┌─────────────────┐        ┌──────────────────────────────┐
    │    core.py      │  ←──   │  kit_aNN.py (um por atividade)│
    │  (mecânica)     │        │  painel + dados + checagens   │
    └─────────────────┘        └──────────────────────────────┘
                                          ↑
                               atividade-NN-<slug>.ipynb

> 👋 **Se você é aluno:** o arquivo que interessa a você é o `kit_aNN.py` da
...
def semente(matricula, tema):
    ...
    O ``tema`` é o identificador da atividade (``"a01"``, ``"a02"``, ...).
    Entra no resumo criptográfico junto com a matrícula para que **cada
    atividade sorteie de forma independente**: quem pegou o negócio mais
    fácil na Atividade 01 não pega necessariamente o mais fácil na 02.
    """
...
def perturbar(valor, ger, amplitude, minimo=None, maximo=None):
    ...
    Os limites preservam plausibilidade: um churn nunca passa de 100%, um
    faturamento nunca é negativo. Perturbação sem limite produz enunciado
...
class Kit:
    ...
    Cada ``kit_aNN.py`` monta uma instância e reexporta os métodos como
    ...
    ----------
    atividade
        Identificador curto e estável: ``"a01"``. Entra na semente, no nome do
    ...
    def iniciar(self, matricula, nome, escopo=None):
        ...
        # independentes. A partir da Atividade em que o aluno tiver listas,
        # ele vai mutar listas em exercícios — sem esta cópia, mutar a lista
        # global do notebook corromperia silenciosamente o gabarito interno
        # do kit, porque ambos apontariam para a MESMA lista na memória.
        ...
        print(f"Fundador(a): {self._dados['NOME']} ({self._dados['MATRICULA']})")
        print()
